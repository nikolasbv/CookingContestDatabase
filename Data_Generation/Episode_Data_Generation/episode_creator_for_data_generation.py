...
def run_episode_creator(file_path="", file_name = "insert_contest_data.sql"):
    ...
    def select_episode_data_with_restrictions(
        ...
        # data for query 10 (was not needed in the end since random data are enough for the query)
        excluded_nationalities = set()
        '''special_episodes_4a = [42, 44, 48, 49]
        special_episodes_4b = [41, 43, 45, 46, 47, 50]
        special_episodes_5a = [52, 54, 56, 58, 59]
        special_episodes_5b = [51, 53, 55, 57, 60]
        special_episodes_6a = [61, 63, 65, 68]
        special_episodes_6b = [62, 64, 66, 67, 69, 70]
        special_episodes_7a = [72, 74, 78, 79]
        special_episodes_7b = [71, 73, 75, 76, 77, 80]
        special_episodes_8a = [82, 84, 88, 89]
        special_episodes_8b = [81, 83, 85, 86, 87, 90]
        
        excluded_nationalities = set()
        special_nationality_1 = 61   
        special_nationality_2 = 77  
        special_nationality_3 = 177 
        special_nationality_4 = 178
        
        if next_episode_id in special_episodes_4a:
            excluded_nationalities.add(special_nationality_1)
        elif next_episode_id in special_episodes_4b:
            selected_nationality_ids.add(special_nationality_1)

        if next_episode_id in special_episodes_5a:
            excluded_nationalities.add(special_nationality_1)
            excluded_nationalities.add(special_nationality_2)
        elif next_episode_id in special_episodes_5b:
            selected_nationality_ids.add(special_nationality_1)
            selected_nationality_ids.add(special_nationality_2)

        if next_episode_id in special_episodes_6a:
            excluded_nationalities.add(special_nationality_2)
            excluded_nationalities.add(special_nationality_3)
        elif next_episode_id in special_episodes_6b:
            selected_nationality_ids.add(special_nationality_2)
            selected_nationality_ids.add(special_nationality_3)
            
        if next_episode_id in special_episodes_7a:
            excluded_nationalities.add(special_nationality_3)
            excluded_nationalities.add(special_nationality_4)
        elif next_episode_id in special_episodes_7b:
            selected_nationality_ids.add(special_nationality_3)
            selected_nationality_ids.add(special_nationality_4)
            
        if next_episode_id in special_episodes_8a:
            excluded_nationalities.add(special_nationality_4)
        elif next_episode_id in special_episodes_8b:
            selected_nationality_ids.add(special_nationality_4)'''

        selected_nationalities = []
        available_nationalities = list(set(nationality_with_recipe_df['nationality_id']) - selected_nationality_ids - excluded_nationalities)
        ...
        # data for query 5
        special_episodes_1a = [12, 14, 18, 19]
        special_episodes_1b = [11, 13, 15, 16, 17, 20]
        special_episodes_2a = [22, 24, 28, 29]
        special_episodes_2b = [21, 23, 25, 26, 27, 30]
        special_episodes_3a = [31, 33, 35, 38, 39]
        special_episodes_3b = [32, 34, 36, 37, 40]
        
        if next_episode_id in special_episodes_1a:
            excluded_cooks = {3}
            special_judges = [3]
            excluded_judges = set()
        elif next_episode_id in special_episodes_1b:
            excluded_cooks = set()
            special_judges = []
            excluded_judges = {3}
        elif next_episode_id in special_episodes_2a:
            excluded_cooks = {4, 5}
            special_judges = [4, 5]
            excluded_judges = set()
        elif next_episode_id in special_episodes_2b:
            excluded_cooks = set()
            special_judges = []
            excluded_judges = {4, 5}    
        elif next_episode_id in special_episodes_3a:
            excluded_cooks = {6, 7}
            special_judges = [6, 7]
            excluded_judges = set()
        elif next_episode_id in special_episodes_3b:
            excluded_cooks = set()
            special_judges = []
            excluded_judges = {6, 7}  
        else:
        ...
        for nationality_id in selected_nationalities:
            available_cooks = list(set(nationality_cook_df[nationality_cook_df['nationality_id'] == nationality_id]['cook_id']) - selected_cook_ids - excluded_cooks)
        ...
        judges = []
        if special_judges:
            for judge_id in special_judges:
                judges.append(judge_id)
                selected_judge_ids.add(judge_id)

        available_judges = list(set(cook_df['cook_id']) - selected_cook_ids - selected_judge_ids - excluded_judges)
    ...
    def generate_episode_image(season, episode, width=600, height=400, bg_color=(144, 238, 144)):
        ...
        try:
            font_season = ImageFont.truetype("arial.ttf", size=60)  # Larger font for the season
            font_episode = ImageFont.truetype("arial.ttf", size=40)  # Smaller font for the episode
        except IOError:
    ...
    def insert_all_queries(user, password, host, port, database,
        ...
        with open(file_path + 'insert_contest_data.sql', 'a') as sql_file:
    ...
    error_variable = insert_all_queries(
        user='root',
        password='',
        host='localhost',
        port='3306',
    ...
    #In success return 0 else return 1 earlier
    return 0
...
def main():
    ...
    with open(file_name, 'a') as file:
        file.write('-- SQL commands generated by episode_creator_for_data_generation\n')
